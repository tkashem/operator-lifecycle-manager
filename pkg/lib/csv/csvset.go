...
// NewClusterServiceVersionSet returns a new instance of ClusterServiceVersionSet.
func NewClusterServiceVersionSet(logger *logrus.Logger, lister operatorlister.OperatorLister) SetGenerator {
	return &csvSet{
		logger: logger,
		lister: lister,
	}
...
// ClusterServiceVersionSet is an interface that returns a map of
// ClusterServiceVersion objects that match a certain set of criteria.
//
// csvSet gathers all CSVs in the given namespace into a map keyed by CSV name;
// if metav1.NamespaceAll gets the set across all namespaces
...
// GetByNamespace returns all ClusterServiceVersion resource(s) from a given
// namespace.
